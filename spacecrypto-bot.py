...
str_in = """
    ...
    +++++++++++++ ETH BITCOIN BUSD BNB ++++++++++++
    + 0x210cfeBe623909e4e67b6dE658dD008b6246bA9f ++
    ++++++++++++++++ Pix key ++++++++++++++++++++++
    ++++ e1324c00-cae3-4d72-a0d3-5fec5f87b078 +++++

    * Algumas configurações podem ser alteradas em 
    settings.yaml

    >> Ctrl + c para finaliza o bot.
...
def refreshPage():
    clickBtn(images['refresh-page'],timeout=1)
    dbg.console('Refresh Page ' + str(st['refreshSeconds']) + ' seconds', 'INFO', 'ambos')
...
def main():
    ...
    f = '%H:%M:%S'
    login_attempts = 0
    dateBreak = time.strftime("%H:%M:%S", time.localtime())
    ...
    while True:
        ...
        action_found = False
        dif = (datetime.strptime(time.strftime("%H:%M:%S", time.localtime()), f) - datetime.strptime(dateBreak, f)).total_seconds()
        ...
        if action_found == False:
            dbg.console('Nenhuma acao encontrada nos ultimos ' + str(int(dif)) + ' segundos', 'WARNING', 'ambos')
